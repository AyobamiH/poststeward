...
**Status: restricted staging. Owner Google sign-in is live-accepted. Inspect-only HTTP/remote MCP grant and revocation passed in an owner-executed live run. Threads app credentials are deployed, but saving the Meta redirect allowlist is blocked; the PostSteward OAuth callback/code-exchange path remains open. The first controlled Threads publication and independent readback passed using a manually imported authorised token; see [P0 evidence](docs/p0-live-evidence-2026-09-12.md). X and LinkedIn applications remain unconfigured. Private GitHub authority, PITR coordination, native WebMCP integration and Stripe billing are engineered but still require their separate live acceptance journeys. Advanced, MPP and public signup remain disabled.**
