...
test("staging request reuses the same commit with only explicitly named secret fallbacks", () => {
  ...
  assert.doesNotMatch(
    request,
    /secrets: inherit|contents: write|actions: write|workflow_dispatch|production|runs-on:/,
  );
  assert.match(
    request,
    /permissions:\n  contents: read\n  pull-requests: read\n/,
...
test("deployment still verifies before entering the protected environment", () => {
  ...
  assert.ok(
    verification.includes(
      "Require merged-PR provenance for automatic staging deployment",
    ),
  );
  assert.ok(
...
test("provider application identifiers remain variables while provider secrets enter only the deploy step", () => {
  for (const name of ["X", "THREADS", "LINKEDIN"]) {
    assert.match(
      deploy,
      new RegExp(
        `${name}_OAUTH_CLIENT_ID: \\\${{ vars\\.${name}_OAUTH_CLIENT_ID }}`,
      ),
    );
    assert.match(
      deploy,
      new RegExp(
        `${name}_OAUTH_CLIENT_SECRET: \\\${{ secrets\\.${name}_OAUTH_CLIENT_SECRET }}`,
      ),
    );
  }
  const verification = deploy.split("  verify:\n")[1].split("  deploy:\n")[0];
