...
- **Native WebMCP:** accepted from an authenticated supporting browser using read-only `workspace_status`, `accounts_list`, `projects_list` and `receipts_list` calls bound to exact production release `7a7f0fa698b1f21b01c5ad2971f13174d4249def`.
...
| Free publishing           | Explicit project routing, immutable campaigns, multipart X/Threads reply chains, publish now, explicit schedules, cancel/replace, receipts, export and on-demand metrics                                                    |
| Agent review              | Every agent-created delivery freezes account, stable identity, exact parts and time in `pending_approval`; only a signed-in owner can release it, and the original agent grant is rechecked at dispatch                       |
