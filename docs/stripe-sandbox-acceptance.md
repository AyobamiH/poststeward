...
The runtime application requires Price reads, Checkout Session creation/list/retrieval/expiry, subscription list/read/cancel authority for billing-safe owner erasure, including expanded invoice/payment/charge data, charge reads for dispute/refund routing, Billing Portal configuration list/read and Billing Portal session creation. It does not need Billing Portal configuration write, webhook-administration or refund-write authority. Setup and refunds use operator authority separately. Explicit workspace erasure expires open Checkout sessions, cancels mapped workspace subscriptions without proration or a new invoice, and confirms terminal state before deleting billing records. A missing cancellation permission keeps deletion pending; it never permits unverified erasure. MPP stays disabled.
