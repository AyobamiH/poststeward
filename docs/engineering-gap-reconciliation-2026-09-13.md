...
| Root replacement | PR48: complete supplied-inventory validation and rewrap rehearsal for account, OAuth and GitHub credentials | Tests cover partial inventories, leases, unknown families, context/key failures and empty inventories. Protected enumeration, mixed-root runtime reads, bounded conditional commits and deployment integration are now implemented; see [protected root cutover](protected-root-cutover.md). A successful protected live run still requires the distinct next root and explicit activation. Supplied-snapshot rehearsal alone is not live inventory proof. |
