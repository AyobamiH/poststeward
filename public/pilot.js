    ...
    for (const element of form.elements) {
      if (element.dataset?.provider) {
        ...
        element.disabled =
          busy ||
          !session ||
          Boolean(snapshot?.record?.deliveryId) ||
          !available;
      } else {
        element.disabled =
          busy || !session || Boolean(snapshot?.record?.deliveryId);
      }
    }
  $("confirm").disabled =
    busy || !session || dirty || !mayApprove(snapshot, $("approve").checked);
  $("approve").disabled =
    busy || dirty || Boolean(snapshot?.record?.deliveryId);
  $("cancel").disabled =
    busy ||
    !["scheduled", "waiting_container"].includes(snapshot?.delivery?.status);
...
function renderOAuth() {
  ...
  const connections = (oauthInfo.connections || []).map((item) => {
    const refresh = item.needsReauthorization
...
function render() {
  ...
  if (snapshot?.owner) {
    ...
    $("owner-status").textContent =
      `Google sign-in completed at ${new Date(o.authenticatedAt).toISOString()}. Proof ${o.id}. Workspace ${o.workspace}.`;
    $("signin").textContent = "Sign in again for a fresh approval";
  }
  $("review-panel").hidden = !r;
  if (r) {
    const readback =
      r.account.capabilities?.readback === true
        ? " · independent readback"
        : "";
    $("destination").textContent =
      `${r.account.provider} · ${r.account.alias} · @${r.account.identity.username} · stable ID ${r.account.identity.id} · binding ${r.account.version}${readback}`;
    $("exact-copy").textContent = r.text;
    $("review-meta").textContent =
      `Review ${r.id}. Expires ${new Date(r.expiresAt).toISOString()}. Runtime ${r.release}. Exact-copy SHA-256 ${r.textDigest}.`;
    $("confirm-form").hidden = Boolean(r.deliveryId);
  }
  $("receipt-status").textContent =
    receiptLabel(snapshot) +
    (d
      ? ` Delivery ${d.id}. Due ${new Date(d.dueAt).toISOString()}.${d.reason ? " Reason: " + d.reason : ""}`
      : "");
  $("receipt").textContent = snapshot
  ...
  if (href) {
    ...
    link.href = href;
    link.textContent = "Open the recorded provider post";
    link.target = "_blank";
    link.rel = "noopener noreferrer";
    $("post-link").append(link);
  }
  renderOAuth();
  controls();
}
async function loadReceipt() {
  ...
  if (previous !== snapshot.record?.id) {
...
async function loadAccounts() {
  ...
  if ([...$("account").options].some((o) => o.value === chosen))
...
async function act(fn) {
  ...
  try {
    ...
    notice(
      error.message ||
        "Request interrupted. Refresh the receipt; do not create a second publication.",
      true,
    );
    if (error.status === 401 || error.code === "OWNER_SIGNIN_REQUIRED") {
      ...
      snapshot = undefined;
      $("owner-status").textContent =
        "A current Google owner sign-in is required. No new approval has been recorded here.";
    } else if (session) {
  ...
  timer = setTimeout(async () => {
    ...
    await act(async () => {
      ...
      if (!snapshot.completed && mayReadback(snapshot)) {
        snapshot = await api("/api/pilot/recheck", {});
        render();
      }
    });
    poll();
...
for (const button of $("oauth-buttons").querySelectorAll(
...
$("connection-form").onsubmit = (event) => {
  ...
  act(async () => {
    try {
      ...
      if (provider === "x" && !$("connection-funding").checked)
        throw new Error(
          "Confirm ownership and API usage of your funded X application first.",
        );
      await api("/api/connections/import", {
        alias: $("connection-alias").value,
        provider,
        accessToken: $("connection-token").value,
        ...(provider === "x" ? { funding: "customer_app" } : {}),
      });
      dirty = true;
      $("approve").checked = false;
      await loadAccounts();
      ...
      await loadReceipt();
      notice(
        "Account identity verified. Manual import does not claim automatic refresh. Choose an eligible destination and prepare the exact review; nothing has posted.",
      );
    } finally {
...
$("prepare-form").onsubmit = (event) => {
  event.preventDefault();
  act(async () => {
    snapshot = await api("/api/pilot/prepare", {
      alias: $("account").value,
      text: $("copy").value,
    });
    dirty = false;
    ...
    render();
    notice(
      "Review prepared without a provider write. Check the stable account ID and every word, then explicitly approve.",
    );
...
$("confirm-form").onsubmit = (event) => {
  ...
  act(async () => {
    snapshot = await api("/api/pilot/confirm", input);
    $("approve").checked = false;
    render();
    notice(
      "One durable delivery reserved. The thirty-second cancellation window has started. Do not create another publication as a retry.",
    );
    polls = 0;
    poll();
  });
...
for (const id of ["account", "copy"])
  $(id).oninput = () => {
    dirty = true;
    $("approve").checked = false;
    controls();
    if (snapshot?.record && !snapshot.record.deliveryId)
      notice(
        "Inputs changed. Prepare a new review; the displayed exact review has not changed.",
      );
  };
$("refresh").onclick = () =>
  act(async () => {
...
$("cancel").onclick = () =>
  act(async () => {
    snapshot = await api("/api/pilot/cancel", {});
    render();
    notice(
      snapshot.cancellation?.cancelled
        ? "Cancellation won before dispatch. No replacement will be created."
        : "Cancellation did not win. Inspect the existing delivery; do not assume publication was stopped.",
    );
  });
$("recheck").onclick = () =>
  act(async () => {
...
$("export-receipt").onclick = () => {
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = "poststeward-acceptance-" + snapshot.record.id + ".json";
  link.click();
...
await act(async () => {
  ...
  if (connected)
    notice(
      `${connected} authorization completed and the provider identity was verified. Review the destination before preparing content.`,
    );
  else if (denied === "denied")
    notice(
      "Provider authorization was declined. No connection or publication was created.",
      true,
    );
  else
    notice(
      snapshot.completed
        ? "The recorded controlled publication has independent provider readback evidence."
        : "Owner sign-in verified. Choose the destination and review the exact publication before approval.",
    );
