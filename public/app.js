...
async function api(path, input, method = input === undefined ? "GET" : "POST") {
  ...
  try {
    ...
    throw new Error(
      "The response was incomplete. Inspect the existing receipt before retrying.",
    );
  }
  if (!response.ok) {
...
function show(data, error = false) {
  showFeedback(data, error, feedbackTarget);
}
...
function navigateExternal(value, hosts) {
  ...
  if (!destination)
    throw new Error(
      "The external destination was not on the expected provider host.",
    );
...
function renderRecovery() {
  ...
  $("recovery-status").textContent = recovery
...
async function refresh() {
  ...
  if (billing.attempt || entitlement || billing.recoveryPending) {
    const receipt = {
      ...
      price: billing.price,
      webhookEvidence: billing.webhookEvidence || {
        available: false,
        events: [],
      },
      evidenceBoundary:
        "Payment state may be reconciled by refresh. Webhook events are a separate persisted ledger captured before that reconciliation; only non-null completedAt records completed processing. Latest 20 events only; replay is not established by this receipt.",
    };
