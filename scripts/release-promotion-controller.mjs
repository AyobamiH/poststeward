...
import {
  assessObservation,
  assessReleaseBinding,
  evaluatedObservation,
  promotionExitCode,
  validateReviewedLedger,
...
const coreGates = new Set([
  "owner_google_signin",
  "threads_publication_readback",
  "inspect_http_remote_mcp",
  "stripe_sandbox_lifecycle",
  "protected_root_cutover",
  "private_github_authority",
...
const actionCatalog = Object.freeze({
  approximate_timestamp_pitr: {
    class: "external_wait",
    action:
      "keep_exact_checkpoints_primary_until_cloudflare_timestamp_resolution_recovers",
  },
  ...
  x_token_refresh_rotation: {
    class: "owner_provider",
    action: "observe_safe_x_token_refresh_rotation",
  },
  linkedin_oauth: {
    class: "owner_provider",
...
async function fetchReadiness(origin, send = fetch) {
  ...
  demand(
    response.status === 200,
    `Hosted readiness returned HTTP ${response.status}.`,
  );
...
export function resolveExpectedRelease({
  mode,
  ...
  const resolved =
    explicit || (mode === "report" ? hostedRelease : githubSha || "");
  demand(
...
export function evaluatePromotion({
  ...
  const binding = assessReleaseBinding({
    ledger,
    readiness,
    expectedRelease,
    target,
  ...
  const policyViolations = Array.isArray(readiness?.policy?.violations)
    ...
    : [];
  const policyHealthy =
    readiness?.policy?.healthy === true &&
    Array.isArray(readiness?.policy?.violations) &&
    policyViolations.length === 0;
  const assessments = Object.fromEntries(
    reviewed.map((gate) => [
      gate.id,
      assessObservation(observations[gate.id], readiness, now),
    ]),
  );
  const requiredOpen = reviewed.filter((gate) =>
    requiredForTarget(gate, target),
  );
  ...
  const optionalActions = reviewed
    .filter(
      (gate) => !acceptedStates.has(gate.state) && !requiredIds.has(gate.id),
    )
    .map(action)
    .filter(Boolean);
  ...
  return {
    ...
    providerContracts: Object.fromEntries(
      ["x", "threads", "linkedin"].map((provider) => [
        provider,
        {
          ...
          requiredScopes:
            readiness?.providers?.[provider]?.requiredScopes || [],
          optionalScopes:
            readiness?.providers?.[provider]?.optionalScopes || [],
    ...
    promotion: {
      ...
      evidenceReady:
        ...
        requiredWithEvidence.every((entry) => entry.evidenceReady),
      reviewRequired: requiredWithEvidence
        .filter((entry) => entry.evidenceReady)
...
export async function collectObservations(env) {
  ...
  const files = [
    [
      "advanced_rollout",
      env.POSTSTEWARD_SLO_OBSERVATION,
      evaluateSloObservation,
    ],
    [
      "capacity_cost_calibration",
      env.POSTSTEWARD_CAPACITY_OBSERVATION,
      capacityObservation,
    ],
    [
      "operational_alert_delivery",
      env.POSTSTEWARD_ALERT_EVIDENCE,
  ...
  if (env.POSTSTEWARD_PRODUCTION_ORIGIN || env.CLOUDFLARE_ZONE_NAME) {
    ...
    demand(
      env.POSTSTEWARD_PRODUCTION_ORIGIN === env.POSTSTEWARD_ORIGIN,
  ...
  if (
    ...
    demand(
      ...
      "Cross-tenant observation needs the exact expected release.",
    );
...
async function main() {
  ...
  demand(
    ["report", "enforce"].includes(mode),
    "Promotion mode must be report or enforce.",
  );
  const origin = exactOrigin(
    env.POSTSTEWARD_ORIGIN ||
      "https://poststeward-staging.woeinvests.workers.dev",
  );
  ...
  const initial = evaluatePromotion({
    target,
    ledger,
    readiness,
    expectedRelease,
  ...
  const observations =
    initial.releaseBinding.ready && initial.runtimePolicy.healthy
      ? await collectObservations({
          ...env,
          POSTSTEWARD_ORIGIN: origin,
          POSTSTEWARD_EXPECTED_RELEASE: expectedRelease,
        })
      : {};
  ...
  for (const secret of [
    ...
    demand(
      ...
      "Promotion report attempted to emit protected material.",
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href)
  main().catch(() => {
    console.error(
      "POSTSTEWARD_PROMOTION_CONTROLLER_FAILED invalid_input_or_observation",
    );
    process.exitCode = 1;
  });
