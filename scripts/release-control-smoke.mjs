export async function verifyReleaseControl(origin, release, send = fetch) {
  ...
  await check(
    "runtime readiness exposes healthy typed release gates",
    "/readiness.json",
    (value, response) =>
      ...
      !value.evidenceStillExternal.includes("native_webmcp"),
  );

  await check(
    "generated public release gate ledger matches reviewed evidence",
    "/release-gates.json",
    (value) => {
      const gates = Object.fromEntries(
        (value.gates || []).map((gate) => [gate.id, gate]),
      );
      return (
        ...
        gates.native_webmcp?.state === "live_verified" &&
  ...
  return {
    ...
    boundary:
      "Read-only release/capability reconciliation. Configuration never upgrades a reviewed evidence state.",
  };
}
