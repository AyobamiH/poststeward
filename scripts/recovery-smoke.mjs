...
export async function inspectRecoveryBoundary(origin, expectedRelease, send = fetch) {
  ...
  async function check(name, path, expected, init = {}, inspect = async () => true) {
    ...
    try {
      ...
      // Fixed names/status only. Never print response bodies, OAuth state,
      // cookies, recovery bookmarks or upstream exception contents.
    }
    checks.push({ name, status, expected, passed });
  }

  await check(
    "exact runtime still healthy before recovery checks",
    "/health",
    200,
    {},
    async (response) => {
  ...
  await check(
    "cross-origin recovery status rejected",
    "/api/recovery/status",
    403,
  ...
  for (const action of [
    ...
    await check(
      `unauthenticated recovery ${action} rejected`,
      `/api/recovery/${action}`,
      401,
      {
        method: "POST",
  ...
  await check(
    "unauthenticated provider OAuth status rejected",
    "/api/connections/oauth/status",
    401,
  );
  for (const provider of ["x", "threads", "linkedin"])
    await check(
      `unauthenticated ${provider} OAuth start rejected`,
      `/api/connections/oauth/${provider}/start`,
      401,
      {
        method: "POST",
        headers: { "Content-Type": "application/json", Origin: origin },
        body: '{"alias":"smoke","returnPath":"/app"}',
      },
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  demand(
    report.passed,
    "Recovery/OAuth hosted boundary checks failed. No destructive restore was attempted.",
  );
}
