...
export async function waitForRevision(
  origin,
  release,
  { send = fetch, sleep = delay, attempts = 12 } = {},
) {
  httpsUrl(origin, true);
  demand(/^[a-f0-9]{40}$/.test(release), "An exact release SHA is required.");
  demand(
    Number.isInteger(attempts) && attempts > 0 && attempts <= 12,
    "Readiness attempts must be bounded.",
  );
  let lastStatus = 0;
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    if (response) {
      demand(
        ...
        "Readiness redirected or rejected access; not retrying.",
      );
  ...
  throw new Error(
    `Expected revision did not become ready within the bounded checks (last HTTP status ${lastStatus}).`,
  );
}
...
export async function verifyHosted(c, { send = fetch, sleep = delay } = {}) {
  ...
  for (const path of ["/", "/app"])
    await check(
      `HTML serves without a redirect: ${path}`,
      path,
      200,
      {},
      async (r) =>
        ...
        (await r.text()).includes("PostSteward"),
    );
  for (const path of [
    "/style.css",
    "/app.js",
  ...
  await check(
    ...
    401,
    {},
    (r) => r.headers.get("cache-control")?.includes("no-store"),
  );
  await check(
    "forged bearer rejected through D1",
    "/api/session",
    401,
    { headers: { Authorization: "Bearer invalid-deployment-probe" } },
    async (r) => (await r.json()).error?.code === "UNAUTHENTICATED",
  );
  await check("cross-origin session rejected", "/api/session", 403, {
    headers: { Origin: "https://untrusted.example" },
  });
  await check("unauthenticated MCP rejected", "/mcp", 401, {
    method: "POST",
  ...
  await check(
    ...
    400,
    {},
    async (r) => (await r.json()).error?.code === "LOGIN_STATE_INVALID",
  );
  await check(
    "OIDC login initiation, PKCE, nonce and secure state cookie",
    "/auth/login",
    302,
    {},
    (r) => {
      ...
      return (
        url.protocol === "https:" &&
        (c.vars.OIDC_ISSUER !== "https://accounts.google.com" ||
          url.hostname === "accounts.google.com") &&
        q.get("client_id") === c.vars.OIDC_CLIENT_ID &&
        q.get("redirect_uri") === origin + "/auth/callback" &&
        q.get("response_type") === "code" &&
        q.get("scope") === "openid profile email" &&
        q.get("code_challenge_method") === "S256" &&
        Boolean(q.get("state") && q.get("nonce") && q.get("code_challenge")) &&
        cookie.includes("__Host-login=") &&
