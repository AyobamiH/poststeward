...
export async function readProviderBody(
  ...
  const stopped = new Promise<never>((_, reject) => {
    abort = () => reject(new Error("Provider deadline"));
  });
...
export function safeThreadsUrl(value: unknown): string | undefined {
  ...
  try {
    ...
    if (
      ...
      !u.password &&
      !u.port &&
      /^\/@[A-Za-z0-9._-]+\/post\/[A-Za-z0-9_-]+\/?$/.test(u.pathname)
    ) {
      u.search = "";
      u.hash = "";
      return u.href;
    ...
    /* An untrusted permalink is not navigation authority. */
  }
}
...
export class SocialProviders implements ProviderAPI {
  ...
  private async request(
    ...
    try {
      ...
      throw new Fault(
        write ? "AMBIGUOUS_PROVIDER_WRITE" : "PROVIDER_INVALID_RESPONSE",
        write
          ? "Provider accepted the request but durable response evidence is unavailable. Do not resubmit."
          : "Provider returned unreadable or incomplete data.",
        502,
      );
    }
  }
  async identity(
    ...
    if (provider === "x") {
      requireValue(
        !actorUrn,
        "IDENTITY_UNVERIFIED",
        "X does not accept a LinkedIn actor URN.",
        400,
      );
      ...
      requireValue(
        validId(data.data?.id) && validId(data.data?.username),
        "IDENTITY_UNVERIFIED",
    ...
    if (provider === "threads") {
      requireValue(
        !actorUrn,
        "IDENTITY_UNVERIFIED",
        "Threads does not accept a LinkedIn actor URN.",
        400,
      );
      const { data } = await this.request(
      ...
      requireValue(
        validId(data.id) && validId(data.username),
        "IDENTITY_UNVERIFIED",
    ...
    if (actorUrn) {
      ...
      // Community Management cannot share an application with OpenID Sign-In.
      // Its role-gated author finder is therefore the identity proof for the
      // exact state-bound Page: success proves this member grant can read it.
      ...
      const { data: authored } = await this.request(
        finder.href,
        c.accessToken,
        {
          headers: {
  ...
  async publish(d: Delivery, c: Credential): Promise<Published> {
    if (d.provider === "x") {
      ...
      return {
        id: data.data.id,
        url: `https://x.com/i/web/status/${encodeURIComponent(data.data.id)}`,
      };
    }
  ...
  async verify(d: Delivery, c: Credential) {
    requireValue(
      validId(d.postId),
      "NO_READBACK_TARGET",
      "A durable provider ID is required.",
      409,
    );
    if (d.provider === "x") {
      const { data } = await this.request(
      ...
      return {
        verified:
          data.data?.id === d.postId &&
          data.data?.author_id === d.identity.id &&
    ...
    const { data } = await this.request(
      `https://graph.threads.net/v1.0/${encodeURIComponent(d.postId)}?fields=id,text,owner,username,permalink`,
      c.accessToken,
    );
    return {
      verified:
        data.id === d.postId &&
        data.owner?.id === d.identity.id &&
        data.text === d.text,
  ...
  async metrics(d: Delivery, c: Credential): Promise<unknown> {
    ...
    if (d.provider === "linkedin")
      return {
        availability: "unavailable",
        reason: "LinkedIn analytics permissions have not been enabled.",
      };
    try {
      if (d.provider === "x") {
        const { data } = await this.request(
