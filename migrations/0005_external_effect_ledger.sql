...
CREATE TABLE external_containers (
  ...
  fingerprint TEXT NOT NULL,
  delivery_id TEXT NOT NULL,
  status TEXT NOT NULL CHECK (status IN ('intent','uncertain','created')),
  container_id TEXT,
  reason TEXT,
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL,
...
CREATE INDEX external_containers_workspace_status ON external_containers(workspace, status);
